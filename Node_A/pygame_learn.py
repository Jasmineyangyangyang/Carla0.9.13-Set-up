...
# set the spectator's view == driver's view
transform_human = ego_vehicle.get_transform()
camera_bp_human = world.get_blueprint_library().find('sensor.camera.rgb')
camera_trasform_human = carla.Transform(carla.Location(x=1.6, z=1.7))
camera = world.spawn_actor(camera_bp_human, camera_trasform_human, attach_to = ego_vehicle)
spectator.set_transform(carla.Transform(transform_human.location + carla.Location(x=-0.22,y=0.2, z=-2.7),
                                        transform_human.rotation))
